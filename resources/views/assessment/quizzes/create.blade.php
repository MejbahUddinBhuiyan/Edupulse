...
<div class="max-w-3xl mx-auto py-8 px-4">
    <div class="mb-6">
        ...
        <p class="text-sm text-gray-500">Add a new quiz for a course.</p>
    </div>

    <div class="bg-white shadow rounded-xl p-6">
        <form action="{{ route('assessment.quizzes.store') }}" method="POST" class="space-y-5">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div>
                    ...
                    <input type="number" step="0.01" name="pass_marks" value="{{ old('pass_marks') }}"
                           class="w-full border rounded-lg px-3 py-2">
                    @error('pass_marks')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
